    ...
    if validation.is_valid_cve_id(cve):
        ...
        if fr"We found it CVE in 'https://nvd.nist.gov/vuln/detail/{cve}'" in result:
            ...
            with open(f'{cve}.txt', 'w') as file:
                file.write(output)
            return render_template('result.html', cve_id=id_vuln, severity=severity_func, description=description_func, source_url=ref_links, vector_func=vector_func, relat_exp=relat_exp)
        elif r"We found it CVE in 'https://services.nvd.nist.gov/rest/json/cves/2.0'" in result:
            cve_id, sourceIdentifier, descriptions1, cvss_score, severity, access_vector, access_complex, authentication, confidentialityImpact, integrityImpact, availabilityImpact = vuln.cve_parse_json(cve)
            relat_exp = vuln.related_exp(cve)
            relat_exp_str = ', '.join(map(str, relat_exp))
            ...
            with open(f'{cve}.txt', 'w') as file:
                file.write(output)
            return render_template('result.html', cve_id=cve_id, severity=severity, description=descriptions1, source_url=sourceIdentifier, access_vector=access_vector, access_complex=access_complex, authentication=authentication, confidentialityImpact=confidentialityImpact, integrityImpact=integrityImpact, availabilityImpact=availabilityImpact, relat_exp=relat_exp)
